...
class OrganizationCode(object):
    ...
    def get_an_organization_code(self):
        ...
        logger.info('生成组织机构代码证 {0}'.format(organization_code))
        return organization_code

    def check_organization_code(self):
        """检查组织机构代码证是否正确，如果正确，返回True，否则返回False"""
        if len(self.bref) != 8 or len(self.check) != 1:
            logger.info('组织机构代码证错误!（本体或校验码长度不符合要求）')
            return False
        else:
            try:
                c9_right = self._get_c9()
                if self.check != c9_right:
                    logger.info('组织机构代码证错误!（校验码错误）')
                    return False
                else:
                    logger.info('验证通过，组织机构代码证格式正确！')
                    return True
            except ValueError:
                logger.info('组织机构代码证错误!(本体错误)')
                return False
